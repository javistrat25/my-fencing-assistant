...
export default async function handler(req, res) {
  ...
  try {
    ...
    const opportunitiesResponse = await axios.get('https://services.leadconnectorhq.com/opportunities/search', {
      ...
      params: { location_id: 'hhgoXNHThJUYz4r3qS18', limit: 100, pipeline_stage_id: quotePendingStageId }
    });
    ...
    quotePendingOpportunities.forEach((opp, index) => { console.log(`${index + 1}. ${opp.name} - Contact: ${opp.contact?.name || 'No contact'}`); });
    res.status(200).json({
      success: true, opportunities: quotePendingOpportunities, total: quotePendingOpportunities.length,
      stageId: quotePendingStageId, message: 'Filtered by pipeline_stage_id (no pagination)'
    });
  } catch (error) {
    console.error('Error fetching quote pending opportunities:', error.response?.data || error.message);
    res.status(500).json({ error: 'Failed to fetch quote pending opportunities', details: error.response?.data || error.message });
  }
} 